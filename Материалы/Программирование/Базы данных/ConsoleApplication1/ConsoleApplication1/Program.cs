    ...
    class Program
    {
        static void Main( string[] args )
        {
            ...
            using (var connection = new SQLiteConnection(connectionString))
            {
                ...
                for (int i = 1; i < 7; i++)
                {
                    comGroups += Insert(tableName,columnNames,new[] { i.ToString(),i+"03" });
                }
               ...
               /*  command.CommandText = "SELECT count(Sciences.Id) FROM 'Sciences';";
                ...
                command.CommandText = @"select Groups.Name, count(Students.GroupId) from Groups join Students on Groups.Id = Students.GroupId group by Groups.Name";
        ...
        {
            var collumnName = string.Join(",",columnNames.Select(x => string.Format("'{0}'", x)));
            var columnValue = string.Join(",",columnValues.Select(x => string.Format("'{0}'", x)));
